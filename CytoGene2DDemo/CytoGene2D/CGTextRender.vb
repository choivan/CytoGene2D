...
Public Class CGTextRender
    ...
    Private centerFormat_ As StringFormat
    ...
    Private postParagraphSpace_ As Single = 15
    ...
    Sub New()
        ...
        centerFormat_ = New StringFormat
        centerFormat_.Alignment = StringAlignment.Center
        centerFormat_.LineAlignment = StringAlignment.Center
    End Sub
    ...
    Public Sub render(g As Graphics, attributedLine As List(Of AttributedString),
        ...
        If aString.attribute = FontAttribute.FontAttributeTitle1 Then
            ...
            g.DrawString(aString.content, title1Font, blackBrush_, layoutArea, centerFormat_)
        ...
        ElseIf containAttribute(aString.attribute, FontAttribute.FontAttributeImage) Then
            ' a image will not reset the ordered list number
            Dim image As New Bitmap(aString.content)
            ' center the image
            g.DrawImage(image, New RectangleF(New PointF(CGDirector.sharedDirector.canvasWidth / 2 - image.Size.Width / 2, layoutArea.Location.Y), image.Size))
            currentStartLocation_.X = layoutArea.Left
    ...
    Private Function getSizeOfParagraphWithConstraintSize(g As Graphics,
        ...
        ElseIf aString.attribute = FontAttribute.FontAttributeTitle2 Then
            Dim title1Font As New Font(fontName_, fontSize_ + kTitle2FontSizeOffset, FontStyle.Bold)
            Dim s As SizeF = g.MeasureString(aString.content, title1Font, constraintSize, stringFormat_)
            Return New SizeF(s.width, s.Height + postParagraphSpace_)
        ElseIf aString.attribute = FontAttribute.FontAttributeTitle3 Then
            Dim title1Font As New Font(fontName_, fontSize_ + kTitle3FontSizeOffset, FontStyle.Bold)
            Dim s As SizeF = g.MeasureString(aString.content, title1Font, constraintSize, stringFormat_)
            Return New SizeF(s.width, s.Height + postParagraphSpace_)
        ElseIf containAttribute(aString.attribute, FontAttribute.FontAttributeImage) Then
            ...
            End If
            Return New SizeF(constraintSize.Width, imageSize.Height + nextSize.Height + postParagraphSpace_)
        Else
        ...
        Return New SizeF(constraintSize.Width + orderSymbolWidth,
                         newSize.Height + postParagraphSpace_)
    End Function
