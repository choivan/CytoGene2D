Module CGHelper
    ...
    Public Sub formEllipseSingleStrand(startNode As CGDNANode, size As Integer,
                                       startDegree As Single, angle As Single, a As Single, b As Single, center As PointF, Optional animated As Boolean = True)
        Dim aDelta As Single = angle / size
        For i As Integer = 0 To size - 1
            If startNode Is Nothing Then Exit For
    ...
    Public Function delayedInstantAction(delay As Integer, instant As CGActionInstant.instantActionDelegate) As CGAction
        Return New CGSequence(New CGDelayTime(delay),
                              New CGActionInstant(instant))
    End Function

    Public Function cgpAdd(origin As PointF, delta As PointF) As PointF
        Return New PointF(origin.X + delta.X, origin.Y + delta.Y)
    End Function
End Module
